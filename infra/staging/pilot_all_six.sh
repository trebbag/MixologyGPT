...
if [[ "${RUN_SIGNOFF}" == "true" ]]; then
  ...
  REQUIRED_PAIRS+=("ALERT_CONFIRM_URL:${ALERT_CONFIRM_URL}")
  REQUIRED_PAIRS+=("INTERNAL_TOKEN:${INTERNAL_TOKEN}")
  REQUIRED_PAIRS+=("SLACK_WEBHOOK_URL:${SLACK_WEBHOOK_URL}")
  REQUIRED_PAIRS+=("PAGERDUTY_ROUTING_KEY:${PAGERDUTY_ROUTING_KEY}")
fi
...
if [[ "${RUN_SIGNOFF}" == "true" && -n "${PAGERDUTY_ROUTING_KEY}" && "${PAGERDUTY_ROUTING_KEY}" == "dummy" ]]; then
  PRECHECK_ITEMS+=("- FAIL: \`PAGERDUTY_ROUTING_KEY\` is set to \`dummy\`")
  MISSING=1
fi
...
if [[ "${ALLOW_LOCAL_ENDPOINTS}" != "true" ]]; then
  ...
  if [[ "${RUN_SIGNOFF}" == "true" ]]; then
    ...
    LOCAL_CHECK_PAIRS+=("ALERT_CONFIRM_URL:${ALERT_CONFIRM_URL}")
    LOCAL_CHECK_PAIRS+=("SLACK_WEBHOOK_URL:${SLACK_WEBHOOK_URL}")
