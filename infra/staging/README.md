...
2. Copy `.env.staging.example` to `.env.staging` and set real values.
   ...
   - Downstream incident destinations are optional:
...
4. Optional external confirmation polling:
   - Confirm receiver received the alert (internal/in-app path):
     - `ALERTMANAGER_URL=http://localhost:9093 CONFIRM_BASE_URL=http://localhost:5001 ./external_alert_smoke.sh`
   - Confirm receiver forwarded to a downstream destination (optional):
...
Use this when you have real staging endpoints. Internal alert validation is enough for sign-off:
```bash
...
Optional: validate external forwarding in the same run:
```bash
cd /Users/gregorygabbert/Documents/GitHub/BartenderAI
API_BASE_URL=https://<staging-host> \
...
INTERNAL_TOKEN=<internal-token> \
RUN_EXTERNAL_FORWARD_SMOKE=true \
